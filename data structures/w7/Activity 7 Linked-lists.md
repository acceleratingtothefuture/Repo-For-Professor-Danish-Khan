...
struct Node {
    int data;                //  actual integer value we store in this node
    Node* next;              // pointer that tells us where the next node is
};
...
void insertAtStart(int value) {
    ...
    newNode->next = head;        // make this new node point to  old head node
