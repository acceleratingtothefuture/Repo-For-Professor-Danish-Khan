...
So for each element of N elements, we just compare it, see that it doesn't need to be swapped, and continue. So for each N, we always do one thing.
...
Before we look at the worst case scenario, let's recall the sum of natural numbers from 1 to N. So in the best case, we can just add one for each N. 
...
But in big O, we don't care about coeffecients or lesser degrees. We care about the N<sup>2</sup>. Therefore, average case is N<sup>2</sup>. 
