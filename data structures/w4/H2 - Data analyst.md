...
All-negative values: a typical algorithim would just output the number closest to zero. But if we're in sales, zero is a minimum. If we assume sales can't be negative and that the sales are distributed around a positive number, we will always have a relatively even ratio of negatives to positive. However, if we did have all negatives for whatever reason (maybe they wanted to look at profit instead of sales), this algorithim would still find the least of those negatives, which would default to the highest number anyway. The sum of two negatives is always further from zero than the lowest between either of those individual negatives. For example, -1 + -1 is more negative than -1 by itself. -4 by itself is better than -4 + -6. We could also introduce another subtraction to account for cost. 
